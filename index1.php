<!DOCTYPE html>
<html>
   <head>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width, initial-scale=1">
      <title>Bootstrap Nav</title>
      <link rel="stylesheet" href="bootstrap/bootstrap.min.css">
   </head>
   <body>
       <header>
          <div class="container">
            <div class="row"> 
                <div class="col-md-4 col-lg-4">
                    <img class="img-fluid" src="images/photo.jpg" width="1000px" height="200px"/>
                </div>
            </div>
           </div>
       </header>
      <div class="container">
         <ul class="nav">
            <li class="nav-item">
                <a class="nav-link" href="Web/Landing.html">Shopping</a>
            </li>
            <li class="nav-item">
               <a class="nav-link" href="Web/Cart.php">My cart</a>
            </li>
            <li class="nav-item">
               <a class="nav-link" href="">Profile</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="Web/Register.php">Register</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="Web/Login.php">Login</a>
            </li>
             <li class="nav-item">
                 <a class="nav-link" href="Web/LogOut.php">Logout</a>
            </li>
         </ul>

      </div>
 
      <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js"></script>
      <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.3/umd/popper.min.js"></script>
      <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.1.1/js/bootstrap.min.js"></script>
   </body>
</html>